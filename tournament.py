...
def newTournament(tournament_name="", sport=""):
    db = connect()
    cursor = db.cursor()
    cursor.execute("INSERT INTO tournaments (tournament_name, sport) \
                   VALUES(%s, %s);", (tournament_name, sport))
    db.commit()
    cursor.execute("SELECT tournament_id FROM tournaments ORDER BY \
                   tournament_id DESC")
...
def registerPlayer(player_name, tournament_id=""):
    ...
    Args:
      ...
      tournament_id: the unique id of the tournament the player is in, not
      required at this point, will have to build some way to provide ref error.
    ...
    if tournament_id != "":
        cursor.execute("INSERT INTO Players (player_name, tournament_id) \
                       VALUES(%s, %s);", (player_name, tournament_id))
    else:
        cursor.execute("INSERT INTO Players (player_name, tournament_id) \
                       VALUES(%s, %s);", (player_name, 0))
    db.commit()
    cursor.close()
    db.close()
...
def playerStandings(tournament_id=""):
    ...
    if tournament_id == "":
        cursor.execute("SELECT id, player_name, wins, matches FROM scores \
...
def reportMatch(winner, loser, tournament_id=""):
    """Records the outcome of a single match between two players.
        Args:
            ...
            tournament_id: the unique id of the tournament the player is in,
            not required at this point, will have to build some way to provide
            ref error.
    """
    db = connect()
    cursor = db.cursor()
    # Different decisiions based on the tournament id.
    # Using 0 as a standard blank tournament that's included in tournament.sql
    if tournament_id != "":
        cursor.execute("INSERT INTO Matches (winner, loser, tournament_id) \
